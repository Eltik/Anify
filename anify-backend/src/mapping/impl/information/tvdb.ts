...
import { Anime, Character, Format, Manga, Season } from "../..";
...
export default class TVDB extends InformationProvider<Anime | Manga, AnimeInfo | MangaInfo> {
    ...
    override async info(media: Anime | Manga): Promise<AnimeInfo | MangaInfo | undefined> {
        ...
        const data: Response | undefined = await this.request(
            ...
            try {
                console.log(colors.red("TVDB Error: " + JSON.stringify(err?.response?.data)) + err?.response?.status);
                console.log(colors.red("Request URL: ") + colors.yellow(`${this.tvdbApiUrl}${tvdbId}/extended`));
            } catch (e) {
                console.log(colors.red("TVDB Error: " + err));
            }
            return undefined;
        });
        ...
        if (data.ok) {
            ...
            const characters: Character[] = (info.characters ?? []).map((character) => {
                return {
                    ...
                    voiceActor: {
                        name: character.peopleName,
                        image: character.peopleImageURL,
                    },
                };
            });
            ...
            const coverImage = hasPrequelRelation ? coverImages[0]?.image || img || null : media.coverImage?.length ?? 0 > 0 ? media.coverImage : coverImages[0]?.image || img || null;
            ...
            console.log(colors.red("TVDB Error: " + JSON.stringify(await data.json())));
        }
