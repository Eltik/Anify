...
export default class AniList extends InformationProvider<Anime | Manga, AnimeInfo | MangaInfo> {
    ...
    public async searchAdvanced(query: string, type: Type, formats: Format[], page: number, perPage: number, genres: Genres[] = [], genresExcluded: Genres[] = [], year = 0, tags: string[] = [], tagsExcluded: string[] = []): Promise<AnimeInfo[] | MangaInfo[] | undefined> {
    ...
    public async fetchSeasonal(type: Type, formats: Format[]) {
        ...
        const trending = data.trending.media?.map((data: Media) => {
            return anilistMediaGenerator(data);
        });
    ...
    public async fetchSchedule(): Promise<Schedule[] | undefined> {
