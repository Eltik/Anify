...
import { Format, Genres, MediaStatus, Season, Sort, SortDirection, Type } from "./types/enums";
...
before().then(async (_) => {
    ...
    const page1 = await searchAdvanced("", Type.MANGA, [Format.MANGA], 1, 100, [Genres.ROMANCE], [], Season.UNKNOWN, 0, [], []);
    const page2 = await searchAdvanced("", Type.MANGA, [Format.MANGA], 2, 100, [Genres.ROMANCE], [], Season.UNKNOWN, 0, [], []);
    
    for (const item of page1) {
        if (page2.find((x) => x.id === item.id)) {
