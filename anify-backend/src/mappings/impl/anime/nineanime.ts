...
export class NineAnimeHandler {
    // Modified from https://github.com/Claudemirovsky/worstsource-keys/blob/master/index.mjs

    private synchrony = new Deobfuscator();
    private webcrackStep = async (x: string) => await (await webcrack(x)).code;
    private synchronyStep = async (x: string) => await this.synchrony.deobfuscateSource(x);
    private checkDeobfs = (x: string) => x.indexOf("<video />") !== -1;

    constructor() {
        //
    }

    public async fetchKeys(): Promise<string[]> {
        const current = await this.fetchLocalKeys();
        if (current.version == this.getCodeVersion()) {
            return current.keys;
        }

        const deobfuscated = await this.getDeobfuscatedScript();
        if (this.checkDeobfs(deobfuscated)) {
            ...
            console.log(colors.green("Successfully extracted 9anime keys."));
            ...
            // ... Or not xD
            console.error(colors.red("Unable to extract 9anime keys!"));
    ...
    public async fetchLocalKeys(): Promise<{ keys: string[]; version: string }> {
        const file = Bun.file("keys.json");
    ...
    private async deobfuscationLoop(obfuscatedInput: string, loopFunction: (x: string) => any) {
    ...
    private async deobfuscationChain(obfuscatedScript: string, deobfsSteps: ((x: string) => any)[]) {
        let deobfs = obfuscatedScript;
        for (const func of deobfsSteps) {
            deobfs = await this.deobfuscationLoop(deobfs, func);
        }
        return deobfs;
    }
    ...
    private async getDeobfuscatedScript() {
        ...
        const scriptUrl = `${vidplayHost}/assets/mcloud/min/embed.js?v=${this.getCodeVersion()}`;
        const obfuscatedScript = await fetch(scriptUrl, { headers: headers }).then(async (x) => await x.text());

        const firstTry = await this.deobfuscationChain(obfuscatedScript, [this.webcrackStep, this.synchronyStep]);
        if (this.checkDeobfs(firstTry)) return firstTry;

        const secondTry = await this.deobfuscationChain(obfuscatedScript, [this.synchronyStep]);
        return secondTry;
    }
    ...
    private modifyURL(token: string, encodedURL: string) {
        const k = token,
            a = [k];
    ...
    private _rc4(key: string, str: string) {
        var s = [];
        var j = 0;
        var x;
        var res = "";
