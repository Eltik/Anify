...
const Read: NextPage<Props> = ({ chapterNumber, chapterSelector, downloadLink, mixdrop, media, nextChapter, pages, previousChapter, readId, provider }) => {
        ...
        <>
            ...
            <main className="flex flex-col">
                {media.format === Format.NOVEL ? (
                    <div>
                        <div
                            className={`header fixed z-10 flex w-[100vw] flex-row items-center justify-between bg-[#181818d2] pt-[5px] transition-all duration-200 ease-in-out ${!showNavbar ? "-translate-y-5 opacity-0" : "translate-y-0 opacity-100"}`}
                            style={{
                        ...
                        >
                            ...
                            <div className="leading-0 line-clamp-3 overflow-hidden text-xl sm:line-clamp-2 md:text-2xl"></div>
                            <div className="flex flex-row items-center justify-center gap-1 text-white">
                                <div className="flex w-full flex-row items-center justify-center">
                                    <div className="flex flex-row-reverse gap-1">
                                        <Link href={previousChapter} rel="noopener nofollow noreferrer" className="pointer accent relative flex min-h-[2rem] min-w-[2rem] items-center overflow-hidden rounded bg-zinc-600 !px-0 text-white transition-all duration-200 hover:bg-background-light">
                                            <span className="pointer-events-none relative flex w-full select-none items-center justify-center font-medium">
                                        ...
                                        <Link href={nextChapter} rel="noopener nofollow noreferrer" className="pointer accent relative flex min-h-[2rem] min-w-[2rem] items-center overflow-hidden rounded bg-zinc-600 !px-0 text-white transition-all duration-200 hover:bg-background-light">
                                            <span className="pointer-events-none relative flex w-full select-none items-center justify-center font-medium">
                                ...
                                />
                                <ChaptersPanel contentSelector={chapterSelector} showChapters={showChapters} />
                            </div>
                        </div>
                        <div className={`mx-auto mt-5 flex w-[90%] flex-col md:w-[40%] ${open || showChapters ? "opacity-50 blur-sm" : "opacity-100 blur-none"} z-0 transition-all duration-200`}>
                            <br />
                            <div
                                dangerouslySetInnerHTML={{
                                    __html: pages as string,
                                }}
                                className={`mt-5 bg-[var(--bg-color)] text-justify text-lg font-extralight text-white md:text-left`}
                                style={{
                            ...
                            ></div>
                            <span className="text-lg text-white">{provider === "JNovels" ? "No data available! Visit the open tab to download the novel. If you can't see it, please allow pop-ups and refresh the page." : ""}</span>
                        </div>
                    </div>
                ) : (
                    <div className="w-full">
                        ...
                        >
                            ...
                            <div className="flex flex-row items-center justify-center gap-1 text-white">
                                {downloadLink?.result[mixdrop ?? ""]?.url ? (
                                    <a href={downloadLink?.result[mixdrop ?? ""]?.url ?? ""} target="_blank" className="ml-auto mr-[12px] flex h-[32px] w-[32px] cursor-pointer items-center justify-center rounded-sm bg-[hsla(0,0%,76%,0)] p-[5px] transition-all duration-200 hover:bg-[hsla(0,0%,76%,.2)]">
                        ...
                        <div className="mt-10 flex flex-col">
                            {pages && typeof pages !== "string" && pages.length > 0 ? (
                                pages.map((page, index) => <MangaPage source={page.url} key={index} />)
                            ) : (
                                <div className="flex flex-col items-center justify-center">
                                    ...
                                    <div className="text-white">
                                        Please try a different connector. If this is a bug, please join our{" "}
                                        <Link href="/discord" className="text-main-text transition-all duration-200 hover:text-main">
                                            Discord
                                        </Link>{" "}
                                        and report it.
                                    </div>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
            ...
            if (providerChap?.providerId.toLowerCase() === (provider as string).toLowerCase()) {
                ...
                if (chapter?.id === readId) {
                    if (j < providerChap.chapters.length - 1) {
                        nextChapter = `/read/${(id as string) ?? ""}/${(provider as string) ?? ""}/${encodeURIComponent(providerChap.chapters[j + 1]?.id ?? readId)}`;
                    } else {
                        nextChapter = `/read/${(id as string) ?? ""}/${(provider as string) ?? ""}/${encodeURIComponent(readId)}`;
                    }
                    if (j > 0) {
                        previousChapter = `/read/${(id as string) ?? ""}/${(provider as string) ?? ""}/${encodeURIComponent(providerChap.chapters[j - 1]?.id ?? readId)}`;
                    } else {
                        previousChapter = `/read/${(id as string) ?? ""}/${(provider as string) ?? ""}/${encodeURIComponent(readId)}`;
                    }
                }
            }
        }
    ...
    const pages = (await (
        await axios.post(String(env.BACKEND_URL) + "/pages?apikey=" + String(env.API_KEY), {
            chapterNumber,
            providerId: provider,
            readId,
            id,
