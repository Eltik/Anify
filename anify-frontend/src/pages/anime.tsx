                    ...
                    {watchTime?.length > 0 ? (
                        <div className="mx-auto mt-5">
                            ...
                            <Swiper slidesPerView={"auto"} className="relative h-fit" freeMode={true}>
                                {watchTime?.reverse().map((media, index: number) => {
                                    return (
...
export const getServerSideProps: GetServerSideProps = async () => {
    const seasonal = (await axios.get(String(env.BACKEND_URL) + "/seasonal/anime?apikey=" + String(env.API_KEY))).data as Seasonal;
